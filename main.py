...
class CharacterId:
    _counter = count(0)
    def __init__(self): self.value = next(CharacterId._counter)
    def __repr__(self): return f"CharacterId({self.value})"

class EventAct: pass
class Narration(EventAct):
    def __repr__(self): return "Narration()"

class CharacterAct: pass
class Speak(CharacterAct):
    def __repr__(self): return "Speak()"
class Imagine(CharacterAct):
    def __init__(self, events: List["Event"]): self.events = events
    def __repr__(self): return f"Imagine(events={self.events!r})"

class CharacterEvent(EventAct):
    def __init__(self, character_id: CharacterId, act: CharacterAct):
        self.id = character_id; self.act = act
    def __repr__(self): return f"CharacterEvent(id={self.id!r}, act={self.act!r})"

class Event:
    def __init__(self, body: str, act: EventAct):
        self.body = body; self.act = act
    def __repr__(self): return f"Event(body={self.body!r}, act={self.act!r})"

def narrate(body: str) -> Event: return Event(body, Narration())

class Character:
    def __init__(self, name: str):
        self.id = CharacterId(); self.name = name
    ...
    def think(self, thought: str) -> Event: return self.imagine(thought, [])
    def __repr__(self): return f"Character(id={self.id!r}, name={self.name!r})"
...
class EventJSON(BaseModel):
    ...
    imagined: Optional[List[SubEventJSON]] = Field(default=None, description="Only for event_type=imagine; 1-3 items")
...
prefix_fn = build_transformers_prefix_allowed_tokens_fn(tok, parser)  # will force valid JSON. :contentReference[oaicite:5]{index=5}
...
def format_thread(thread: List[Event], char_lookup: Dict[int,str]) -> str:
    return "\n".join(render_event(e, char_lookup) for e in thread[-16:])  # last 16 turns
...
def predict_one(thread: List[Event], characters: List[Character], temperature=0.7, max_new_tokens=220):
...
thread = [
    sparkle.speak("And what a *pleasure* to see you here. *You look very charming today.*"),
    narrate("She grinned at you, looking you up and down. Then, with a devious gleam in her eyes, Sparkle leaned forward - a hand on her hip."),
    narrate("She leaned even closer, her voice teasing."),
    sparkle.speak("Say... I heard the most *interesting* rumours about you. Care to confirm it?"),
    caelus.think("Sparkle... Fancy seeing you here. Rumours? What is she talking about?"),
    caelus.speak('Oh hey, Sparkle. I don\'t know what these "interesting" rumours you talk about are...'),
]
    ...
    for t, gold_json in eval_threads:
        ...
        try:
            ...
            def event_type_of(e: Event):
                if isinstance(e.act, Narration): return "narration"
                if isinstance(e.act, CharacterEvent):
                    if isinstance(e.act.act, Speak): return "speak"
                    if isinstance(e.act.act, Imagine): return "think" if not e.act.act.events else "imagine"
            if event_type_of(p) == gold_json["event_type"]:
                correct_type += 1
            preds.append(p.body); refs.append([gold_json["body"]])
        except Exception:
            pass
